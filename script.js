...
function checkLocalStorageHealth() {
    try {
        ...
        if (e.name === 'QuotaExceededError') {
            if (confirm('저장 공간이 부족합니다. 모든 데이터를 초기화하시겠습니까?')) {
                localStorage.clear();
                location.reload();
            }
        }
...
function loadData() {
    ...
    if (pending) {
        try {
            ...
            console.error('Failed to parse pending photos:', e);
            state.pendingPhotos = [];
        }
    }
}
...
function loadAdminState() {
    const adminState = localStorage.getItem(STORAGE_KEYS.ADMIN);
    if (adminState === 'true') {
        state.isAdmin = true;
    }
}
...
function generateSamplePhotos() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        // 더 간단한 SVG로 변경
        const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><rect width="${width}" height="${height}" fill="${color}"/><text x="50%" y="50%" font-family="Arial" font-size="20" fill="white" text-anchor="middle" dy=".3em">${year}</text></svg>`;
...
function saveData() {
    try {
        ...
        if (e.name === 'QuotaExceededError') {
            showToast('저장 공간이 부족합니다. 일부 사진을 삭제해주세요.');
...
function handleFile(file) {
    ...
    reader.onload = (e) => {
        // 이미지 리사이징 (최대 600px, 품질 0.6)
        resizeImage(e.target.result, 600, 0.6, (resizedDataUrl) => {
            elements.previewImage.src = resizedDataUrl;
...
function handleClearData() {
    if (!confirm('정말 모든 데이터를 초기화하시겠습니까? 이 작업은 취소할 수 없습니다.')) {
        return;
    }
    ...
    localStorage.removeItem('liked_photos');
...
function createPendingItem(photo) {
    ...
    div.innerHTML = `
        ...
        <div class="pending-actions">
            <button class="btn-approve" data-photo-id="${photo.id}">승인</button>
            <button class="btn-reject" data-photo-id="${photo.id}">거절</button>
        </div>
    `;
    ...
    approveBtn.addEventListener('click', () => approvePhoto(photo.id));
    rejectBtn.addEventListener('click', () => rejectPhoto(photo.id));
...
function createApprovedItem(photo) {
    ...
    div.innerHTML = `
        ...
        <div class="pending-actions">
            <button class="btn-delete" data-photo-id="${photo.id}">삭제</button>
        </div>
    `;
...
// toggleLike는 HTML에서 onclick으로 사용하므로 window에 노출
